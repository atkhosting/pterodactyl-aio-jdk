...
A collection of Docker images for the Pterodactyl Panel, providing multiple JDK vendors in a single container. You can switch between vendors at runtime using the `JDK_VENDOR` environment variable. x86 only for now(arm64 may or maynot be included in these builds, but not supported for now).
